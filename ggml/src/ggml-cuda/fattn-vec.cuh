...
static __global__ void flash_attn_ext_vec(
    ...
    // F8 is grouped with F16/BF16 here: the e4m3 quants are decoded to f16 inside the dot product
    // (vec_dot_fattn_vec_KQ_f8) rather than consumed as integers, so Q must stay f16 as well.
    constexpr bool K_is_float = type_K == GGML_TYPE_F16 || type_K == GGML_TYPE_BF16 || type_K == GGML_TYPE_F8;
    constexpr bool V_is_float = type_V == GGML_TYPE_F16 || type_V == GGML_TYPE_BF16 || type_V == GGML_TYPE_F8;

    constexpr int nthreads    = ggml_cuda_fattn_vec_get_nthreads_device();
    constexpr int nthreads_KQ = K_is_float ? 128 / cpy_nb : nthreads_KQ_q;
    constexpr int nthreads_V  = V_is_float ? 128 / cpy_nb : nthreads_V_q;

    static_assert(WARP_SIZE % nthreads_KQ == 0, "bad nthreads_K");
    static_assert(WARP_SIZE % nthreads_V  == 0, "bad nthreads_V");

    constexpr int V_rows_per_thread = V_is_float ? 2*cpy_ne : 4;
    constexpr int V_cols_per_iter   = WARP_SIZE / nthreads_V;

    constexpr vec_dot_KQ_t vec_dot_KQ = get_vec_dot_KQ<type_K, D, nthreads_KQ>();
    constexpr bool Q_q8_1 = !K_is_float;
...
#define EXTERN_DECL_FATTN_VEC_CASES(D, type_K)             \
    ...
    extern DECL_FATTN_VEC_CASE(D, type_K, GGML_TYPE_F8);   \
...
EXTERN_DECL_FATTN_VEC_CASES( 64, GGML_TYPE_F8)
...
EXTERN_DECL_FATTN_VEC_CASES(128, GGML_TYPE_F8)
...
EXTERN_DECL_FATTN_VEC_CASES(256, GGML_TYPE_F8)
